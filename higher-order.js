...
//Code Here
let totalPopulation = populations.reduce((accumulator,incrementer)=>accumulator+incrementer);
...
let orderTotals = orders.map((element)=>(1+element.tax)*element.price);
var x = orderTotals;
x
...
let bobsTotal = purchases.filter(element=>element.owner=="Bob").reduce((accumulator,incrementer)=>accumulator+incrementer.price,0);
